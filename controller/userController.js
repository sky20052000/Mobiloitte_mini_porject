       ...
       // check valid email 
        const validEmail = Validator.isEmail(email);
         ...
         const isMatchPassword = await find_User.comparePassword(password);
          ...
          // generate token 
          const  token = jwt.sign({id:find_User._id, role:find_User.role}, process.env.Jwt_Secret_Key, { expiresIn: '1h' });
  ...
  try{
        let{email} = req.body;
           ...
           await User.updateOne({email},{$set:{Token:token}});
            ...
            return res.status(200).json({success:true,message:"Email send successfully please check your mail!"})
         ...
         let {password} = req.body;
           const getToken = await User.findOne({Token:req.query.token});
            ...
            const hashPasshword = await bcrypt.hash(password,10);
             await User.findByIdAndUpdate({_id:getToken._id},{$set:{password:hashPasshword, Token:''}},{new:true});
...
const deleteUserByAdmin = async(req,res)=>{
    try{
      const  getUser  = await User.findById({_id:req.params.id});
             ...
             return  res.status(400).json({success:false,message:"User deleted successfully"});
